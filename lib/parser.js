...
define(['underscore', 'lib/solfege'], function (_, solfege) {
    ...
    function tokenizePatternDefinitionRHS (str) {
        ...
        var forms = [
            ...
            {regExp: /^(,)/, type: 'comma'},
            {regExp: /^(\^)/, type: 'sustain'},
            {regExp: /^:(\w[\w\-]*)/, type: 'track-name'},
            {regExp: /^\*(\d+)/, type: 'repeat'},
    ...
    function parsePatternDefinitionRHS (tokenList) {
        ...
        function repeat (n) {
            if (!inList()) {
                throw new Error('Cannot repeat nothing.');
            }
            var currentList = lists[lists.length - 1];
            var template = currentList.value[currentList.value.length - 1];
            for (var i = 1; i < n; ++i) {
                currentList.value.push(_.clone(template));
            }
        }
