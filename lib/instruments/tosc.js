    ...
    return function Tosc (presetName) {
        ...
        this.warmUp = function toscWarmUp (ac, destination, note, sStart, sStop) {
            ...
            var freqs = _.map(addDetunedNotes, function (semitones) {
                return Math.pow(2, - (semitones * 100 / 1200)) * note.freq;
            });
            ...
            function newBuffer () {
                return ac.createBuffer(2, dInSamples, ac.sampleRate);
            }
            ...
            if (!buffers[cacheKey]) {
                ...
                _.each(freqs, function prepareNoteForFrequency (freq, nFreq) {
                    var oscs = makeOscillators(freq, ac.sampleRate);

                    for (var channel = 0; channel < 2; ++channel) {
                        if (nFreq === 0) {
                            buffer.getChannelData(channel)[s] = 0;
                        }
                        for (var s = 0; s < dInSamples; ++s) {
                            buffer.getChannelData(channel)[s] += oscs[channel].getSample() / mixFactor;
                        }
                    }
                });
